(() => {
    ...
    const getPluginBase = () => {
        try {
            if (typeof require === 'function') {
                ...
                if (obsidian?.Plugin) return obsidian.Plugin;
            }
        } catch (_) {}
        return class {};
    };
    ...
    const resolveMarkdownView = (app, eventPath) => {
        ...
        if (
            typeof obsidian !== 'undefined' &&
            obsidian?.MarkdownView &&
            typeof workspace.getActiveViewOfType === 'function'
        ) {
            const typedView = workspace.getActiveViewOfType(obsidian.MarkdownView);
    ...
    class TaskStatesPlugin extends PluginBase {
        ...
        async onload() {
            if (this._handler) {
                ...
                document.removeEventListener(EVENT_TYPE, this._handler, true);
                this._handler = null;
            }
            ...
            if (typeof window !== 'undefined') {
                window.__task_line_test_handler__ = this._handler;
            }
        }
        ...
        onunload() {
            ...
            if (typeof window !== 'undefined' && window.__task_line_test_handler__) {
                window.__task_line_test_handler__ = null;
            }
        }
    }
    ...
    if (typeof window !== 'undefined') {
        const previous = window.__taskStatesTestPlugin;
        if (previous && typeof previous.onunload === 'function') {
            previous.onunload();
        }

        if (typeof app !== 'undefined') {
            const instance = new TaskStatesPlugin(app);
            window.__taskStatesTestPlugin = instance;
            window.__taskLineTest = {
                install: () => instance.onload(),
                uninstall: () => instance.onunload(),
                plugin: instance
            };
            instance.onload();
        }
